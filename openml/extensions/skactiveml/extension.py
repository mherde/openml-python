...
class SkactivemlExtension(SklearnExtension):
    ...
    def model_to_flow(self, model: Any) -> "OpenMLFlow":
        ...
        flow.model['selector_model'] = model['selector_model']
        return flow
    ...
    def _run_model_on_fold(
        ...
        * wall clock time will be measured if the number of cores is set (recursive throughout the
        ...
        selection_model = sklearn.base.clone(model['selector_model'])
        ...
        self._prevent_optimize_n_jobs(selection_model)
        ...
        try:
            ...
            if isinstance(task, OpenMLActiveClassificationTask):
                y = np.full(shape=y_train.shape, fill_value=None)
                for c in range(task.budget):
                    query_idx = query_strategy.query(X=X_train, y=y)
